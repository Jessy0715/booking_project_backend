...
/**
 * Demo 站的種子場地：Studio A ~ J 共 10 間。
 *
 * <p>⚠️ 內容是依契約範例推算的，只有 Studio A 逐字對得上舊版。
 *
 * <p>圖片用 Unsplash 的網址，不是上傳到我們的儲存空間 ——
 * 所以每日重置清空圖片時，這些網址不受影響。
 *
 * <p>放在 room 模組而不是 demo 模組：Room 是這裡的 Entity，
 * 別的模組不該直接組裝它。
 */
final class RoomSeedData {

    private RoomSeedData() {
    }
    ...
    private static Room room(String title, String description, String floor, int area, int capacity,
